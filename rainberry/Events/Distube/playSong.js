...
module.exports = async (client, queue, song) => {
    ...
    let thing = client.embed.distube(client, song.user);
    thing.setThumbnail(song.thumbnail)
    ...
    if (song.playlist) {
        ...
        let embed = await queue.textChannel.send({
            embeds: [ thing ],
            components: [ row ]
        });
    ...
    collector.on('collect', async i => {
        ...
        if (i.customId === 'pauseId') {
            ...
            const embed = new MessageEmbed()
                ...
                .setFooter(`Request by ${i.user.tag} • ${message.client.music.status(queue)}`,i.user.displayAvatarURL());
